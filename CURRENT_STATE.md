...
- V1.2 · Adım 1-4 tamamlandı; Adım 4'teki hybrid PDF P1 düzeltmesi commit bekliyor. Benchmarkta açık kalan P2/P3 başlıkları (döndürülmüş sayfalar, bozuk text layer, ilk 50.000 karakter stratejisi, gürültülü taramada tür kayması) henüz iş olarak açılmadı. V1.2 kapsamına yeni iş açılmadan önce `PROJECT_BRAIN.md` ve `DECISIONS.md` ile birlikte değerlendirilir.
...
- Benchmarkta açık kalan noktalar (V1.2 · Adım 4): 90°/180° döndürülmüş taramalarda OCR anlamsız metin üretiyor ve belge `needs_review`'a düşüyor (otomatik döndürme/OSD kullanılmıyor); 10 karakterden uzun ama bozuk bir text layer OCR'ı engelliyor; belirleyici içerik ilk 50.000 karakterden sonra yer alıyorsa (D-027) Gemini'ye hiç gitmiyor. Üçünde de sonuç `needs_review` olduğu için yanlış atama değil, sınıflandırılamama yaşanıyor. Yoğun gürültülü taramada belge türü kayabiliyor (kurum doğru kalıyor).
- OCR yalnızca PDF'te ve yalnızca **sayfanın kendi** gömülü metni 10 karakterin altındaysa çalışır; normal metin PDF'lerinde ek maliyet yoktur. Tek sayfalık temiz bir taramada ~0,6 sn sürdü, ancak süre sayfa sayısı ve tarama kalitesiyle artar ve senkron isteğin toplam süresine eklenir.
